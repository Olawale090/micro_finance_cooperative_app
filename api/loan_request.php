                ...
                if($data["transaction_start_days"] >= 90){
                    ...
                    if($data["total_deposit"] >=  $minimum_deposit){
                        ...
                        $query ="   SELECT SUM(outstanding_loan_amount) AS all_time_loans,
                                    ...
                                    FROM get_loan 
                                    WHERE bvn = '$bvn' 
                                    ORDER BY id DESC LIMIT 1) AS recent_loan_collected,

                                    (SELECT account_status
                                    FROM get_loan 
                                    WHERE bvn = '$bvn' 
                                    ORDER BY id DESC LIMIT 1) AS account_status,

                                    (SELECT SUM(amount) FROM user_deposit WHERE bvn = '$bvn')AS all_time_deposits
                        ...
                        if(!is_null($data["account_status"])){
                            ...
                            if($data["account_status"] == $customer_not_defualt){
                                ...
                                $account_status = user_acccount_status::CUSTOMER_OWING->value;
                                $outstanding_loan = $this->outstanding_amount;

                                $all_time_loans = $data["all_time_loans"];
                                $all_time_deposits = $data["all_time_deposits"];

                                $amount_in_account = $all_time_deposits - $all_time_loans;
                                ...
                                if(!is_null($loan_payback_date)){
                                    try { 
                                            ...
                                            if($pass_query){
                                                ...
                                                $update_acc_mngr = " UPDATE cooperative_app.user_account_manager
                                                                     SET account_name = '$username', 
                                                                         account_number = '$account_number',
                                                                         bvn = '$bvn',
                                                                         bank_name = '$user_bank',
                                                                         amount_saved = '$amount_in_account',
                                                                         amount_owed = '$outstanding_loan',
                                                                         payment_deadline ='$loan_payback_date',
                                                                         maximum_loan_capacity = '$available_loan_amount',
                                                                         account_status = '$account_status'

                                                                    WHERE bvn = '$bvn'
                                                                    ";

                                                $acc_mngr_data = mysqli_query($this->host,$update_acc_mngr,MYSQLI_USE_RESULT);
                                                ...
                                                if($acc_mngr_data){
                                                    ...
                                                    echo json_encode([
                                                        "success"=>false,
                                                        "error"=>[
                                                            "message"=>"Loan application can not continue, contact customer care" 
                                                        ],
                                                        "loan_status"=>loan_status::LOAN_FAILED,
                                                        "status"=>200
                                                    ]);
                            ...
                            try {
                                    $query = "  SELECT amount AS amount_first_deposited,
                                                transaction_date AS first_transaction_date,
                                                SUM(amount) AS total_deposit
                                                FROM user_deposit
                                                WHERE bvn = '$bvn'
                                    ...
                                    $pass_query = mysqli_query($this->host,$query,MYSQLI_USE_RESULT);
                                    $new_user_data = $pass_query->fetch_assoc();
